...
import { RewardToast } from "@/components/companion/RewardToast";
import { AdventurerXpBar } from "@/components/adventurer/AdventurerXpBar";
import { isIntimateCompanion } from "@/lib/companion/bond";
import { ClickableAdventureAvatar } from "@/components/avatar/ClickableAdventureAvatar";
import { formatMinutes } from "@/lib/utils";
import { computeTodayFocusStats } from "@/lib/pomodoro/stats";
import { type CompanionMood, type CompanionSpecies } from "@/lib/types";
...
export default function DashboardPage() {
  ...
  const todayStats = useMemo(() => computeTodayFocusStats(sessions), [sessions]);
  ...
  return (
    <div className="space-y-5">
      ...
      <SquadScene
        ...
        treatCount={progress?.treatCount ?? 0}
...
function SquadScene({
  ...
  const companionDetail = companion
    ? [isIntimateCompanion(bondXp) ? "親密夥伴" : null, moodLabel].filter(Boolean).join(" · ")
    : undefined;
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <SquadSection
          ...
          name={displayName}
          detail={profile?.aiCharacterUrl ? undefined : "點擊頭像覺醒"}
          stats={<AdventurerXpBar adventurerXp={profile?.adventurerXp ?? 0} />}
        />
        ...
        <SquadSection
          ...
          detail={companion ? companionDetail : "還沒有夥伴"}
          action={
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              {companion && treatCount > 0 && (
                <TreatBadge count={treatCount} species={companion.species} size="xs" />
              )}
              <Link href="/companion" className="inline-flex">
                <span className="text-label text-xs px-3 py-1 rounded-full border-2 border-[var(--pixel-mp)] text-[var(--pixel-border)] bg-gradient-to-b from-[#b8f0eb] to-[var(--pixel-mp)] hover:brightness-105 transition-all leading-none">
                  {companion ? "互動 →" : "選擇夥伴 →"}
                </span>
              </Link>
            </div>
